...
namespace PArtCore.Class
{
    public class Class_Core_News
    {
        ...
        public List<Tbl_News_Type> SelectAll(string parminIds, string siteIDs, string filterNewsIds, bool? HasImage, int? newsType, string search, int? PageNumber, int? PageCount, int? SkipCount, string DateFrom, string DateTo, long lastNewsID, string newsIds)
        {
            ...
            var query = @"select * from (SELECT     ROW_NUMBER() OVER(ORDER BY Tbl_News.NewsId desc) AS RowNum,COUNT(*) OVER() AS TotalRows,Tbl_News.NewsID, Tbl_News.SiteID, Tbl_News.NewsDate,[Tbl_News].[NewsTitleLeadVoice],
            ...
            query += " AND ([Tbl_News].[NewsTitleLeadVoice] IS NULL OR [Tbl_News].[NewsTitleLeadVoice]='')  ";
            ...
            query += " order by NewsDateIndex ASC,NewsId  ASC";
        ...
        public bool UpdateNewsSoundField(int newsID)
        {
            var newsLead= "http://www.media.e-sepaar.net/voice/"+ "titlelead_" + newsID + ".mp3";
            var res = _clsAdo.ExecuteSQL("", "UPDATE [Tbl_News] SET [NewsTitleLeadVoice]='"+ newsLead + "' WHERE NewsID=" + newsID, null);
            if (res>0)
            {
                return true;
