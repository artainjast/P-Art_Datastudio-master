<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Twitter_TopFavorite.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Twitter_TopFavorite" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow">
                <span class="title2">تعداد کل لایک ها</span>
                <span class="count  " id="telegram_top_favorite">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin19" runat="server"  />
<script type="text/javascript">
    function LoadTop_Twitter_Favorite() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin19.ClientID %>").val();
        //alert(toDate);
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Twitter_Mohtava_Favorite.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#telegram_top_favorite').parent().children('.arrow_zero').hide();
                            $('#telegram_top_favorite').parent().children('.arrow').show();

                        }
                        else {
                            $('#telegram_top_favorite').parent().children('.arrow').hide();
                            $('#telegram_top_favorite').parent().children('.arrow_zero').show();

                        }
                        $('#telegram_top_favorite').text(data);
                        $('#telegram_top_favorite').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
