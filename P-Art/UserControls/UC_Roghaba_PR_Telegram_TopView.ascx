<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Telegram_TopView.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Telegram_TopView" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow telbot">
                <span class="title2">تعداد کل بازدید ها</span>
                <span class="count  " id="telegram_top_view">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin16" runat="server"  />
<script type="text/javascript">
    function LoadTop_Telegram_View() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin16.ClientID %>").val();
        //alert(toDate);
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Telegram_Mohtava_View.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#telegram_top_view').parent().children('.arrow_zero').hide();
                            $('#telegram_top_view').parent().children('.arrow').show();

                        }
                        else {
                            $('#telegram_top_view').parent().children('.arrow').hide();
                            $('#telegram_top_view').parent().children('.arrow_zero').show();

                        }
                        $('#telegram_top_view').text(data);
                        $('#telegram_top_view').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
