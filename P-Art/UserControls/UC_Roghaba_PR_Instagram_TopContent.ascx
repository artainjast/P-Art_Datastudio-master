<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Instagram_TopContent.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Instagram_TopContent" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow instaleft">
                <span class="title2">تعداد کل محتوا ها</span>
                <span class="count  " id="instagram_top_content">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin13" runat="server"  />
<script type="text/javascript">
    function LoadTop_Instagram_Content() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin13.ClientID %>").val();
        //alert(toDate);
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Instagram_Mohtava_Content.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#instagram_top_content').parent().children('.arrow_zero').hide();
                            $('#instagram_top_content').parent().children('.arrow').show();

                        }
                        else {
                            $('#instagram_top_content').parent().children('.arrow').hide();
                            $('#instagram_top_content').parent().children('.arrow_zero').show();

                        }
                        $('#instagram_top_content').text(data);
                        $('#instagram_top_content').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
