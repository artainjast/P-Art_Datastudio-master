<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Twitter_TopAllContent.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Twitter_TopAllContent" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow">
                <span class="title2">تعداد کل محتوا</span>
                <span class="count  " id="telegram_top_all">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin18" runat="server"  />
<script type="text/javascript">
    function LoadTop_Twitter_AllContent() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin18.ClientID %>").val();
        //alert(toDate);
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Twitter_Mohtava_All.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#telegram_top_all').parent().children('.arrow_zero').hide();
                            $('#telegram_top_all').parent().children('.arrow').show();

                        }
                        else {
                            $('#telegram_top_all').parent().children('.arrow').hide();
                            $('#telegram_top_all').parent().children('.arrow_zero').show();

                        }
                        $('#telegram_top_all').text(data);
                        $('#telegram_top_all').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
