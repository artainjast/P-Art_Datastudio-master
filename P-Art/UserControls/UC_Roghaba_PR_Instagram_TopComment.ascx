<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Instagram_TopComment.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Instagram_TopComment" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow instaleft">
                <span class="title2">تعداد کل کامنت ها</span>
                <span class="count  " id="instagram_top_comment">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin12" runat="server"  />
<script type="text/javascript">
    function LoadTop_Instagram_Comment() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin12.ClientID %>").val();
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Instagram_Mohtava_Comment.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#instagram_top_comment').parent().children('.arrow_zero').hide();
                            $('#instagram_top_comment').parent().children('.arrow').show();

                        }
                        else {
                            $('#instagram_top_comment').parent().children('.arrow').hide();
                            $('#instagram_top_comment').parent().children('.arrow_zero').show();

                        }
                        $('#instagram_top_comment').text(data);
                        $('#instagram_top_comment').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
