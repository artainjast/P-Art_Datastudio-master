<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_Ads_AberPiade.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_Ads_AberPiade" %>
<figure class="highcharts-figure chartRow container-fluid rtl">
    <div id="Competitors_Ads_AberPiade" class="content_container_preload_min"></div>
</figure>
<asp:HiddenField ID="hddParmin7" runat="server"  />
<script type="text/javascript">
    function Load_Competitors_Ads_AberPiade() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin7.ClientID %>").val();
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Ads_Advertise_ByType.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p + "&type=3",
                data: "",
                ...
                success: function (data) {
                    ...
                    var options = {
                        chart: {
                            ...
                            options3d: {
                                ...
                                load: addTitleAberPiade,
                                redraw: addTitleAberPiade,
                            }
                        },
                        title: {
                    ...
                    $('#Competitors_Ads_AberPiade').highcharts(options);
                }
            });
        }
        catch (ex) {
    ...
    function addTitleAberPiade() {
        ...
        this.title = r.text('پل عابر پیاده', 0, 0)
            .css({
                color: '#4572A7',
                fontSize: '16px'
            }).hide()
