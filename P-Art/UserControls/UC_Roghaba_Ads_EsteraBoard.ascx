<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_Ads_EsteraBoard.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_Ads_EsteraBoard" %>
<figure class="highcharts-figure chartRow container-fluid rtl">
    <div id="Competitors_Ads_EsteraBoard" class="content_container_preload_min"></div>
</figure>
<asp:HiddenField ID="hddParmin8" runat="server"  />
<script type="text/javascript">
    function Load_Competitors_Ads_EsteraBoard() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin8.ClientID %>").val();
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Ads_Advertise_ByType.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p + "&type=2",
                data: "",
                ...
                success: function (data) {
                    ...
                    var options = {
                        chart: {
                            ...
                            options3d: {
                                ...
                                load: addTitleEsteraBoard,
                                redraw: addTitleEsteraBoard,
                            }
                        },
                        title: {
                    ...
                    $('#Competitors_Ads_EsteraBoard').highcharts(options);
                }
            });
        }
        catch (ex) {
    ...
    function addTitleEsteraBoard() {
        ...
        this.title = r.text('استرابورد', 0, 0)
            .css({
                color: '#4572A7',
                fontSize: '16px'
            }).hide()
