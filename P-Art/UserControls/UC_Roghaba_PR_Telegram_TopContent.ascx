<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Telegram_TopContent.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Telegram_TopContent" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow telbot">
                <span class="title2">تعداد کل کانال ها</span>
                <span class="count  " id="telegram_top_content">-</span>
                <div class="arrow">
...
<script type="text/javascript">
    function LoadTop_Telegram_Content() {
        ...
        try {
            $.ajax({
                type: "POST",
                url: '/Services/Service_Competitor.aspx/Competitors_Telegram_Mohtava_AllContent',
                ...
                success: function (data) {
                    if (data.d != null) {
                        ...
                        if (data.d > 0) {
                            $('#telegram_top_content').parent().children('.arrow_zero').hide();
                            $('#telegram_top_content').parent().children('.arrow').show();

                        }
                        else {
                            $('#telegram_top_content').parent().children('.arrow').hide();
                            $('#telegram_top_content').parent().children('.arrow_zero').show();

                        }
                        $('#telegram_top_content').text(data.d);
                        $('#telegram_top_content').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
