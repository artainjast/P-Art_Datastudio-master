<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UC_Roghaba_PR_Telegram_TopChannel.ascx.cs" Inherits="P_Art.UserControls.UC_Roghaba_PR_Telegram_TopChannel" %>
  <figure class="highcharts-figure container-fluid all_rates chartRow telbot">
                <span class="title2">تعداد کل محتوا</span>
                <span class="count  " id="telegram_top_channel">-</span>
                <div class="arrow">
...
<asp:HiddenField ID="hddParmin15" runat="server"  />
<script type="text/javascript">
    function LoadTop_Telegram_Channel() {
        var fromDate = $('#txt_fromDate').val().replace("/", "").replace("/", "");
        var toDate = $('#txt_toDate').val().replace("/", "").replace("/", "");
        var p = $("#<%= hddParmin15.ClientID %>").val();
        //alert(toDate);
        try {
            $.ajax({
                type: "POST",
                url: "/Services/Part_Competitors_Telegram_Mohtava_Channel.ashx?f=" + fromDate + "&t=" + toDate + "&p=" + p,
                data: "",
                ...
                success: function (data) {
                    if (data != null) {
                        ...
                        if (data > 0) {
                            $('#telegram_top_channel').parent().children('.arrow_zero').hide();
                            $('#telegram_top_channel').parent().children('.arrow').show();
                        }
                        else {
                            $('#telegram_top_channel').parent().children('.arrow').hide();
                            $('#telegram_top_channel').parent().children('.arrow_zero').show();
                        }
                        $('#telegram_top_channel').text(data);
                        $('#telegram_top_channel').attr("class", "count animate__animated animate__heartBeat");
                    }
                    else {
                    }
                },
