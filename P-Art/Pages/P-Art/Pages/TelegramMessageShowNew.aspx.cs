...
{
    public partial class TelegramMessageShowNew : System.Web.UI.Page
    {
...
#pragma warning restore CS0219 // The variable 'isRead' is assigned but its value is never used
                ...
                long messageId = long.Parse(RouteData.Values["Id"].ToString());
                message = (from row in _db.Tbl_TLPMessage where row.ID == messageId select row).FirstOrDefault();
                ...
                if (message != null)
                {
                    ...
                    LblNewsBody.InnerHtml = (message.MessageText);
                    lblNewsDate.Text = message.MessageDate + " - " + message.MessageTime;
                    ChannelLabel.Text = GetChannelName((int)message.ChannelID);
                    KeywordTitle.Text = GetKeywordName((int)message.KeywordID);
                    lnk_news.HRef = @"https://t.me/" + GetChannelUserName((int)message.ChannelID) + "/" + message.PostID.ToString();
        ...
        public string GetChannelUserName(int ChannelID)
        {
            string Username = _db.Tbl_TLPChannels.Where(C => C.ChannelID == ChannelID).Select(C => C.Username).FirstOrDefault();
