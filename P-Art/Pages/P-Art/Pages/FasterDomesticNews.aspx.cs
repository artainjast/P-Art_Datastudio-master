...
{
    public partial class FasterDomesticNews : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["IsAdmin"] != null)
            {
                if (bool.Parse(Session["IsAdmin"].ToString()) == true)
                {
                    ...
                    keywordPageButton.HRef = "/keywords";
                }
                else
                {
        ...
        private void LoadGroups(List<Tbl_News_Type> dt, List<Tbl_RssKeywords> keywordList)
        {
        ...
        protected void btn_ShowNews_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("/FasterNewsList/?" + url);
        ...
        public static string NewsRateOpacityNew(object newsValue, string MustValue)
        {
            if (newsValue == null)
            {
                if (MustValue == "0")
        ...
        protected void btn_saveState_Click(object sender, EventArgs e)
        {
            ...
            //set user session
            db.SaveChanges();
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('ذخیره وضعیت با موفقیت ثبت شد')", true);
        ...
        protected void btn_GroupSaveState_Click(object sender, EventArgs e)
        {
            ...
            var userOrderedNews = "";
            var currentUser = Class_Layer.CurrentUser();
            // foreach(RepeaterItem group in news_list)


            foreach (RepeaterItem group in rptGroupNewKeyword.Items)
            {
                Repeater rptGroupNewsList = group.FindControl("rptGroupNewsList") as Repeater;
        ...
        private void LoadData(int siteType)
        {
            ...
            if (siteId != 0)
            {
                if (siteId == -1)
                {
                    SiteIDs = _db.AllSitesView.Where(s => s.SiteType == 1).Select(s => s.SiteID).ToList();
                }
                else if (siteId == -2)
                {
                    SiteIDs = _db.AllSitesView.Where(s => s.SiteType == 2).Select(s => s.SiteID).ToList();
                }
                else
                {
                    SiteIDs = _db.AllSitesView.Where(s => s.SiteID == siteId).Select(s => s.SiteID).ToList();
                }

            }
            else
            {
                SiteIDs = _db.AllSitesView.Select(s => s.SiteID).ToList();
            }
            ...
            if (diffrentDt.Days == 0)
            {
                ...
                {
                    //List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsTodayView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
                        news.ParminId = n.ParminId;
                        news.SiteID = n.SiteID;
                        news.NewsDate = n.NewsDate;
                        news.NewsTime = n.NewsTime;
                        news.NewsTitle = n.NewsTitle;
                        news.CreateDate = n.CreateDate;
                        news.NewsLink = n.NewsLink;
                        news.NewsDateIndex = n.NewsDateIndex;
                        news.KeywordId = n.KeywordId;
                        news.GroupId = n.GroupId;
                        news.NewsLinkCRC = n.NewsLinkCRC;
                        news.IsSelected = n.IsSelected;
                        news.NewsType = n.NewsType != null ? n.NewsType : 1;
                        news.NewsTitleCRC = n.NewsTitleCRC;
                        news.IsFeederNews = n.IsFeederNews;
                        news.NewsGroupOrder = n.NewsGroupOrder != null ? n.NewsGroupOrder : 0;
                        news.KeyIds = n.KeyIds != null ? n.KeyIds : n.KeywordId + "";
                        news.NewsTypeShow = n.NewsTypeShow;
                        news.NewsDateTimeIndex = n.NewsDateTimeIndex;
                        news.SortOrder = n.SortOrder != null ? n.SortOrder : 1000;
                        news.NewsValue = n.NewsValue != null ? n.NewsValue : 0;
                        news.SiteTitle = n.SiteTitle != null ? n.SiteTitle : "";
                        news.SiteType = n.SiteType != null ? n.SiteType : 1;
                        news.KeywordName = n.KeywordName != null ? n.KeywordName : "";
                        news.GroupName = n.GroupName != null ? n.GroupName : "";
                        news.Color = n.Color;
                        news.GroupOrder = n.GroupOrder != null ? n.GroupOrder : 0;
                        news.OrderItem = n.OrderItem != null ? n.OrderItem : 0;
                        news.NewsLead = n.NewsLead != null ? n.NewsLead : n.NewsLead + "";
                        news.NewsPicture = n.NewsPicture != null ? n.NewsPicture : n.NewsPicture + "";
                        news.SiteLogo = n.Logo != null ? n.Logo : n.Logo + "";
                        news.ViewCount = n.ViewCount != null ? (int)n.ViewCount : 0 ;
                        DomesticNewsList.Add(news);
                    }
                }
            }
            else if (diffrentDt.Days == 1)
            {
                ...
                {
                    // List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsFromTwoDaysAgoView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            else if (diffrentDt.Days > 1 && diffrentDt.Days <= 5)
            {
                ...
                {
                    // List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsFromFiveDaysAgoView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            else if (diffrentDt.Days > 5 && diffrentDt.Days <= 10)
            {
                ...
                {
                    //List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsFromTenDaysAgoView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            else if (diffrentDt.Days > 10 && diffrentDt.Days <= 31)
            {
                ...
                {
                    //List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsFromOneMonthAgoView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            else if (diffrentDt.Days > 31 && diffrentDt.Days <= 95)
            {
                ...
                {
                    //List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsFromThreeMonthAgoView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            {
                List<DomesticNewsView> DomesticNewsView = new List<DomesticNewsView>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    //List<DomesticNews_Type> DomesticNewsList = new List<DomesticNews_Type>();
                    var newsList = _db.DomesticNewsView.Where(m => m.ParminId == parminId && m.NewsDateTimeIndex >= fromDateTime && m.NewsDateTimeIndex <= toDateTime && SiteIDs.Contains(m.SiteID)).ToList();
                    foreach (var n in newsList)
                    {
                        DomesticNews_Type news = new DomesticNews_Type();
                        news.NewsID = n.NewsID;
            ...
            if (!string.IsNullOrEmpty(strSearch))
            {
                ...
                List<int> DomesticNewsIdList = new List<int>();
                List<int> SearchResultNewsId = new List<int>();

                DomesticNewsIdList = DomesticNewsList.Select(n => n.NewsID).ToList();

                SearchResultNewsId = SearchPhraseInNews(DomesticNewsIdList, strSearch);

                DomesticNewsList = DomesticNewsList.Where(n => SearchResultNewsId.Contains(n.NewsID)).Select(n => n).ToList();
            }


            List<int> CurrentNewsIDsList = DomesticNewsList.Select(n => n.NewsID).ToList();
            List<Tbl_NewsRelate> RelatedNewsList = (from Related in _db.Tbl_NewsRelate
                                                    where CurrentNewsIDsList.Contains(Related.NewsId.Value) && Related.RelateNewsId != null
                                                    orderby Related.NewsId
                                                    select Related).ToList();
            List<int> NewsRelatedIDs = new List<int>();

            foreach (var item in DomesticNewsList)
            {
                var SelectedGroup = keywordList.Where(g => g.GroupOrder == item.NewsGroupOrder).FirstOrDefault();
                //if (SelectedGroup == null)
                //    continue;
                byte NewsTypeShow = 0;
                NewsRelatedIDs.Clear();
                if (!string.IsNullOrEmpty(item.NewsTypeShow.ToString()))
                {
                    NewsTypeShow = byte.Parse(item.NewsTypeShow.ToString());
                }
                NewsRelatedIDs.AddRange(GenerateRelatedList(item.NewsID, RelatedNewsList));
                GroupNewsList.Add(new Tbl_News_Type
                {
                    KeyIds = item.KeyIds,
                    NewsID = item.NewsID,
                    SiteID = item.SiteID,
                    ...
                    NewsDateIndex = item.NewsDateIndex != null ? (long)item.NewsDateIndex : 0,
                    SiteTitle = item.SiteTitle,
                    SiteType = item.SiteType != null ? (int)item.SiteType : 0,
                    KeywordId = item.KeywordId != null ? (int)item.KeywordId : 0,
                    GroupOrder = SelectedGroup.GroupOrder != null ? SelectedGroup.GroupOrder : 0,
                    GroupId = item.GroupId != null ? (int)item.GroupId : 0,
                    GroupName = SelectedGroup.GroupName != null ? SelectedGroup.GroupName.ToString() : "",
                    NewsLink = item.NewsLink,
                    NewsLink2 = item.NewsLink,
                    NewsLinkCRC = item.NewsLinkCRC != null ? (long)item.NewsLinkCRC : 0,
                    Color = item.Color,
                    IsSelected = item.IsSelected != null ? (bool)item.IsSelected : false,
                    NewsValue = item.NewsValue != null ? item.NewsValue.ToString() : "",
                    IsFeederNews = item.IsFeederNews != null ? (bool)item.IsFeederNews : false,
                    SortOrder = item.SortOrder != null ? item.SortOrder.ToString() : "",
                    CreateDate = item.CreateDate.ToString(),
                    RelateListIds = NewsRelatedIDs,
                    RelatedNewsStringIds = GenerateListToString(NewsRelatedIDs),
                    NewsGroupOrder = item.NewsGroupOrder != null ? item.NewsGroupOrder : 0,
                    OrderItem = item.OrderItem != null ? (int)SelectedGroup.OrderItem : 0,
                    KeywordName = item.KeywordName,
                    NewsTypeShow = NewsTypeShow,
                    NewsKeyTagsOrder = SelectedGroup.OrderItem != null ? Convert.ToInt32(SelectedGroup.OrderItem) : 0,
                    NewsKeyGroupsOrder = SelectedGroup.GroupOrder != null ? Convert.ToInt32(SelectedGroup.GroupOrder) : 0,
                    NewsLead = item.NewsLead != null ? item.NewsLead : item.NewsLead + "",
                    NewsPicture = item.NewsPicture != null ? item.NewsPicture : item.NewsPicture + "",
                    SiteLogo = item.SiteLogo != null ? item.SiteLogo : item.SiteLogo + "" ,
            ...
            Session.Add("keywordList", keywordList);
        ...
        protected void rptRelatedNews_list_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            //HtmlGenericControl hypNewsValueNeg = e.Item.FindControl("hypNewsValueNeg") as HtmlGenericControl;
            //HtmlGenericControl hypNewsValueNoRel = e.Item.FindControl("hypNewsValueNoRel") as HtmlGenericControl;
            //HtmlGenericControl hypNewsValue = e.Item.FindControl("hypNewsValue") as HtmlGenericControl;
            //HtmlGenericControl hypNewsValueRel = e.Item.FindControl("hypNewsValueRel") as HtmlGenericControl;
            //HtmlGenericControl hypNewsValuePos = e.Item.FindControl("hypNewsValuePos") as HtmlGenericControl;

            //if (newsData.NewsValue == null)
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "0.3");
            //    hypNewsValueNoRel.Style.Add("opacity", "0.3");
            //    hypNewsValue.Style.Add("opacity", "1");
            //    hypNewsValueRel.Style.Add("opacity", "0.3");
            //    hypNewsValuePos.Style.Add("opacity", "0.3");
            //}
            //else if (newsData.NewsValue == "" || newsData.NewsValue == "0")
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "0.3");
            //    hypNewsValueNoRel.Style.Add("opacity", "0.3");
            //    hypNewsValue.Style.Add("opacity", "1");
            ...
            //    hypNewsValuePos.Style.Add("opacity", "0.3");
            //}
            //else if (newsData.NewsValue == "1")
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "0.3");
            //    hypNewsValueNoRel.Style.Add("opacity", "0.3");
            //    hypNewsValue.Style.Add("opacity", "0.3");
            //    hypNewsValueRel.Style.Add("opacity", "0.3");
            //    hypNewsValuePos.Style.Add("opacity", "1");
            //}
            //else if (newsData.NewsValue == "2")
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "1");
            //    hypNewsValueNoRel.Style.Add("opacity", "0.3");
            //    hypNewsValue.Style.Add("opacity", "0.3");
            //    hypNewsValueRel.Style.Add("opacity", "0.3");
            //    hypNewsValuePos.Style.Add("opacity", "0.3");
            //}
            //else if (newsData.NewsValue == "3")
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "0.3");
            //    hypNewsValueNoRel.Style.Add("opacity", "0.3");
            //    hypNewsValue.Style.Add("opacity", "0.3");
            //    hypNewsValueRel.Style.Add("opacity", "1");
            //    hypNewsValuePos.Style.Add("opacity", "0.3");
            //}
            //else if (newsData.NewsValue == "4")
            //{
            //    hypNewsValueNeg.Style.Add("opacity", "0.3");
            //    hypNewsValueNoRel.Style.Add("opacity", "1");
            //    hypNewsValue.Style.Add("opacity", "0.3");
            //    hypNewsValueRel.Style.Add("opacity", "0.3");
            //    hypNewsValuePos.Style.Add("opacity", "0.3");
            //}
        ...
        protected void news_list_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            HyperLink hypNewsValueNeg = e.Item.FindControl("hypNewsValueNeg") as HyperLink;
            HyperLink hypNewsValueNoRel = e.Item.FindControl("hypNewsValueNoRel") as HyperLink;
            HyperLink hypNewsValue = e.Item.FindControl("hypNewsValue") as HyperLink;
            HyperLink hypNewsValueRel = e.Item.FindControl("hypNewsValueRel") as HyperLink;
            HyperLink hypNewsValuePos = e.Item.FindControl("hypNewsValuePos") as HyperLink;
        ...
        protected void rbtGroupList_Click(object sender, EventArgs e)
        {
            //var GroupGridIsCreated = CheckGroupGridIsCreated();
            //if (!GroupGridIsCreated)
            //{
                try
                {
                    if (GroupNewsList == null || GroupNewsList.Count == 0)
                    {
                        ...
                        keywordList = (List<Tbl_RssKeywords>)Session["keywordList"];
                    }
                    LoadGroups(GroupNewsList, keywordList);
                    SetGroupGridIsCreated();
                    SetActiveGrid("GroupList");
                    PrepareActiveElement();
        ...
        protected void rbtCommonList_Click(object sender, EventArgs e)
        {
            SetActiveGrid("NewsList");
            PrepareActiveElement();
        }

        private void SetGroupGridIsCreated()
        {
            if (Session["GroupGridIsCreated"] == null)
            {
                Session.Add("GroupGridIsCreated", "true");
            }
            else
            {
                Session["GroupGridIsCreated"] = "true";
            }
        }

        private bool CheckGroupGridIsCreated()
        {
            if (Session["GroupGridIsCreated"] != null)
            {
                if (Session["GroupGridIsCreated"].ToString() == "true")
                {
                    return true;
                }
            }
        ...
        private void SetActiveGrid(string input)
        {
            if (Session["ActiveGrid"] == null)
            {
                Session.Add("ActiveGrid", input);
            }
            else
            {
                Session["ActiveGrid"] = input;
            }
        }

        private string GetActiveGrid()
        {
            if (Session["ActiveGrid"] != null)
                return Session["ActiveGrid"].ToString();
            else
            {
                SetActiveGrid("NewsList");
                return Session["ActiveGrid"].ToString();
        ...
        private void PrepareActiveElement()
        {
            var ActiveGrid = GetActiveGrid();
            if (ActiveGrid == "NewsList")
            {
                rbtCommonList.Style.Remove("color");
                rbtCommonList.Style.Add("color", "#fec006");

                newsSection.Style.Remove("display");
                newsSection.Style.Add("display", "block");

                rbtGroupList.Style.Remove("color");
                groupSection.Style.Remove("display");
                groupSection.Style.Add("display", "none");
            }
            else if (ActiveGrid == "GroupList")
            {
                rbtCommonList.Style.Remove("color");
                rbtGroupList.Style.Remove("color");
                rbtGroupList.Style.Add("color", "#fec006");

                groupSection.Style.Remove("display");
                groupSection.Style.Add("display", "block");

                newsSection.Style.Remove("display");
                newsSection.Style.Add("display", "none");
            }
            else
            {
                rbtCommonList.Style.Remove("color");
        ...
        //protected void ExportExcelReportButton_Click(object sender, EventArgs e)
        //{
        //    string fromDate = "";
        //    string toDate = "";
        //    string toTime = "";
        ...
        //    if (txt_fromDate.Text != "")
        //    {
        //        fromDate = txt_fromDate.Text.Replace("/", "");
        //    }
        //    if (txt_toDate.Text != "")
        //    {
        //        toDate = txt_toDate.Text.Replace("/", "");
        //    }
        //    if (drp_NewsSource.SelectedItem.Value != "0")
        //    {
        //        siteId = drp_NewsSource.SelectedItem.Value;
        //    }
        //    if (txt_search.Text.Trim() != "")
        //    {
        //        keyword = Server.HtmlEncode(txt_search.Text.Trim().Replace("+", "|"));
        //    }
        //    if (txt_fromHour.Text != "" && txt_toHour.Text != "")
        //    {
        //        fromTime = Server.HtmlEncode(txt_fromHour.Text.Trim().Replace(":", "-"));
        //        toTime = Server.HtmlEncode(txt_toHour.Text.Trim().Replace(":", "-"));
        //    }

        //}


        //public void GenerateExcelReport(string fromDate, string toDate, string fromTime, string toTime, string siteId, string keyword)
        //{
