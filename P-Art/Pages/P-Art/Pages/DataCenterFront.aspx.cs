...
{
    public partial class DataCenterFront : System.Web.UI.Page
    {
        ...
        List<DataCenterNewsList> dataCenterNews = new List<DataCenterNewsList>();
        List<Tbl_News_Type> GroupNewsList = new List<Tbl_News_Type>();
        List<Tbl_RssKeywords> keywordList = new List<Tbl_RssKeywords>();
        List<RssKeywordsView> keywordsList = new List<RssKeywordsView>();
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        private void LoadGroupingKeywords(List<string> NewsList)
        {

            List<int> keyList = new List<int>();
            foreach (var keyids in NewsList)
            {
                foreach (var id in keyids.Split(','))
                {
                    if (!string.IsNullOrWhiteSpace(id))
                    {
                        keyList.Add(int.Parse(id.ToString()));
                    }
            ...
            keywordsList = (from keys in _db.RssKeywordsView where keyList.Contains(keys.KeyId) orderby keys.GroupOrder, keys.OrderItem select keys).Distinct().ToList();

            rpt_Groups.DataSource = keywordsList;
            rpt_Groups.DataBind();
        ...
        protected void btn_ShowNews_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("/Pages/P-Art/Pages/datacenterfront.aspx?" + url);
...
#pragma warning restore CS0219 // The variable 'toTimeZone' is assigned but its value is never used
            ...
            {
                fromDateTime = Convert.ToInt64(txt_fromDate.Text.Replace("/", "") + "0000");
            }
            if (!string.IsNullOrWhiteSpace(txt_toDate.Text))
            {
                toDateTime = Convert.ToInt64(txt_toDate.Text.Replace("/", "") + "2400");
            }

            if (!string.IsNullOrWhiteSpace(txt_fromHour.Text))
            {
                fromDateTime = +Convert.ToInt64(txt_fromHour.Text.Replace(":", ""));

            }
            if (!string.IsNullOrWhiteSpace(txt_toHour.Text))
            {
                toDateTime = +Convert.ToInt64(txt_toHour.Text.Replace(":", ""));
            ...
            {
                List<DataCenterNewsListToday> DataCenterNewsListToday = new List<DataCenterNewsListToday>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListToday = (from news in _db.DataCenterNewsListToday
                                               where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                               select news).ToList();

                }

                news_list.DataSource = DataCenterNewsListToday;
                news_list.DataBind();

                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListToday select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else if (diffrentDt.Days == 1)
            {
                List<DataCenterNewsListFromYesterday> DataCenterNewsListFromYesterday = new List<DataCenterNewsListFromYesterday>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListFromYesterday = (from news in _db.DataCenterNewsListFromYesterday
                                                       where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                                       select news).ToList();
                }

                news_list.DataSource = DataCenterNewsListFromYesterday;
                news_list.DataBind();

                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListFromYesterday select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else if (diffrentDt.Days > 1 && diffrentDt.Days <= 5)
            {
                List<DataCenterNewsListFromFiveDaysAgo> DataCenterNewsListFromFiveDaysAgo = new List<DataCenterNewsListFromFiveDaysAgo>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListFromFiveDaysAgo = (from news in _db.DataCenterNewsListFromFiveDaysAgo
                                                         where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                                         select news).ToList();
                }

                news_list.DataSource = DataCenterNewsListFromFiveDaysAgo;
                news_list.DataBind();

                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListFromFiveDaysAgo select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else if (diffrentDt.Days > 5 && diffrentDt.Days <= 10)
            {
                List<DataCenterNewsListFromTenDaysAgo> DataCenterNewsListFromTenDaysAgo = new List<DataCenterNewsListFromTenDaysAgo>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListFromTenDaysAgo = (from news in _db.DataCenterNewsListFromTenDaysAgo
                                                        where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                                        select news).ToList();
                }

                news_list.DataSource = DataCenterNewsListFromTenDaysAgo;
                news_list.DataBind();

                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListFromTenDaysAgo select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else if (diffrentDt.Days > 10 && diffrentDt.Days <= 31)
            {
                List<DataCenterNewsListFromOneMonthAgo> DataCenterNewsListFromOneMonthAgo = new List<DataCenterNewsListFromOneMonthAgo>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListFromOneMonthAgo = (from news in _db.DataCenterNewsListFromOneMonthAgo
                                                         where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                                         select news).ToList();
                }

                news_list.DataSource = DataCenterNewsListFromOneMonthAgo;
                news_list.DataBind();

                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListFromOneMonthAgo select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else if (diffrentDt.Days > 31 && diffrentDt.Days <= 95)
            {
                List<DataCenterNewsListFromThreeMonthAgo> DataCenterNewsListFromThreeMonthAgo = new List<DataCenterNewsListFromThreeMonthAgo>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    DataCenterNewsListFromThreeMonthAgo = (from news in _db.DataCenterNewsListFromThreeMonthAgo
                                                           where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                                           select news).ToList();
                }

                news_list.DataSource = DataCenterNewsListFromThreeMonthAgo;
                news_list.DataBind();


                List<string> listOfNewsKeyId = new List<string>();
                listOfNewsKeyId = (from news in DataCenterNewsListFromThreeMonthAgo select news.KeyIds).Distinct().ToList();
                LoadGroupingKeywords(listOfNewsKeyId);
            }
            else
            {
...
#pragma warning restore CS0219 // The variable 'error' is assigned but its value is never used
        ...
        public string ShowKeywords(string KeyIds)
        {
            string finalElement = "";
            if (string.IsNullOrWhiteSpace(KeyIds))
                return null;
            ...
            foreach (var keySpil in KeyIds.Split(','))
            {
                ...
                {
                    var selKeyId = Convert.ToInt32(keySpil);
                    var key = _db.Tbl_RssKeywords.FirstOrDefault(t => t.KeyId == selKeyId);
                    if (key == null) continue;

                    finalElement += "<a data-keyid='" + key.KeyId + "' data-text='" + key.KeywordName + "'>" + key.KeywordName + "</a>";
            ...
            finalElement += "<input type='hidden' id='hdfNewsTags' name='hdfNewsTags' value='" + KeyIds + "'>";
