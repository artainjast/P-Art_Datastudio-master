...
{
    public partial class TelegramKeywords : System.Web.UI.Page
    {
        ...
        List<Tbl_Telegram_SearchKeyWords> TelegramKey = new List<Tbl_Telegram_SearchKeyWords>();
        ...
        private void prepareDropDownList()
        {
            NewsGroupList = (from Group in _db.Tbl_NewsGroup
                             where UserPanelList.Contains(Group.ParminId) && Group.GroupType == 3
        ...
        protected void OnItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                TelegramKey = (from key in _db.Tbl_Telegram_SearchKeyWords
                               where key.FK_GroupId == keyGroup
                               orderby key.KeyPriority
                               select key).ToList();
                rtpKeyWordItemRepeater.DataSource = TelegramKey;
                rtpKeyWordItemRepeater.DataBind();
            }
        }
        [WebMethod]
        public static Tbl_NewsGroup GetTelegramKeyGroupWithId(string GroupID)
        {
        ...
        [WebMethod]
        public static Tbl_Telegram_SearchKeyWords GetTelegramKeywordWithId(string KeyID)
        {
            int keyId = int.Parse(KeyID);
            var keyWord = _dbStatic.Tbl_Telegram_SearchKeyWords.Where(key => key.Id == keyId).FirstOrDefault();
            return keyWord;
        }
        ...
        protected void deleteGroupButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var Keyword = _db.Tbl_Telegram_SearchKeyWords.Where(key => key.FK_GroupId == groupId).FirstOrDefault();
                if(Keyword == null)
                {
                    var Group = _db.Tbl_NewsGroup.Where(k => k.GroupId == groupId).FirstOrDefault();
                    _db.Tbl_NewsGroup.Remove(Group);
        ...
        protected void saveKeyword_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var keyword = _db.Tbl_Telegram_SearchKeyWords.Where(k => k.Id == keyId).FirstOrDefault();
                if (keyword != null)
                {
                    if (!string.IsNullOrWhiteSpace(txtKeywordTitle.Text))
                        keyword.SearchKeyWord = txtKeywordTitle.Text;
                    ...
                    if (!string.IsNullOrWhiteSpace(lstGroup.SelectedValue))
                        keyword.FK_GroupId = int.Parse(lstGroup.SelectedValue);
                ...
                else
                {
                    if (!string.IsNullOrWhiteSpace(txtKeywordTitle.Text))
                    {
                        _db.Tbl_Telegram_SearchKeyWords.Add(new Tbl_Telegram_SearchKeyWords
                        {
                            SearchKeyWord = txtKeywordTitle.Text,
                            PeriodTime = 1,
                            msg_offset_date = 0,
                            msg_offset_id = 0,
                            DateTimePreviousSearch = "0",
                            DateTimePreviousSearchUnix = 0,
                            FK_GroupId = int.Parse(lstGroup.SelectedValue),
                            FK_ParminId = parmin,
        ...
        protected void deleteKeyword_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var keyword = _db.Tbl_Telegram_SearchKeyWords.Where(k => k.Id == keyId).FirstOrDefault();
                _db.Tbl_Telegram_SearchKeyWords.Remove(keyword);
