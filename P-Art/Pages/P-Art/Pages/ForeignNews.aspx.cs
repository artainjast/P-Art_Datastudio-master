...
{
    public partial class ForeignNews : System.Web.UI.Page
    {
        [ThemeableAttribute(false)]
        public virtual string GroupName { get; set; }

        private static DB_NewsCenterEntities _db = new DB_NewsCenterEntities();
        List<ForeignNews_Type> ForeignNewsList = new List<ForeignNews_Type>();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                lnk_yesterday.HRef = "/ForeignNews/?FromDate=" + date + "&toDate=" + date;
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        protected void btn_ShowNews_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("/ForeignNews/?" + url);
...
#pragma warning restore CS0219 // The variable 'toTimeZone' is assigned but its value is never used
            ...
            else if (diffrentDt.Days > 0 && diffrentDt.Days <= 31)
            {
                List<ForeignNewsFromOneMonthAgoView> ForeignNewsFromOneMonthAgoView = new List<ForeignNewsFromOneMonthAgoView>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    ForeignNewsList = (from news in _db.ForeignNewsFromOneMonthAgoView
                                        where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                        select new ForeignNews_Type
                                        {
                                            NewsID = news.NewsID,
            ...
            {
                List<ForeignNewsView> ForeignNewsView = new List<ForeignNewsView>();
                if (fromDateTime != 0 && toDateTime != 0)
                {
                    ForeignNewsList = (from news in _db.ForeignNewsView
                                        where UserPanelList.Contains(news.ParminId) && news.NewsDateTimeIndex >= fromDateTime && news.NewsDateTimeIndex <= toDateTime
                                        select new ForeignNews_Type
                                        {
                                            NewsID = news.NewsID,
            ...
            if (siteId != 0)
            {
                ForeignNewsList = ForeignNewsList.Where(n => n.SiteID == siteId).Select(n => n).ToList();
            }
            if (!string.IsNullOrEmpty(strSearch))
            {
                strSearch = Class_Static.ArabicAlpha(strSearch);

                List<int> ForeignNewsIdList = new List<int>();
                List<int> SearchResultNewsId = new List<int>();

                ForeignNewsIdList = ForeignNewsList.Select(n => n.NewsID).ToList();

                SearchResultNewsId = _db.Tbl_News.Where(n => ForeignNewsIdList.Contains(n.NewsID) && (n.NewsTitle.Contains(strSearch) || n.NewsLead.Contains(strSearch) || n.NewsBody.Contains(strSearch))).Select(n => n.NewsID).ToList();

                ForeignNewsList = ForeignNewsList.Where(n => SearchResultNewsId.Contains(n.NewsID)).Select(n => n).ToList();
            }


            List<int> CurrentNewsIDsList = ForeignNewsList.Select(n => n.NewsID).ToList();
            ...
            foreach (var item in ForeignNewsList)
            {
                byte NewsTypeShow = 0;
                ...
                GroupNewsList.Add(new Tbl_News_Type
                {
                    ...
                    OrderItem = item.OrderItem != null ? (int)item.OrderItem : 0,
                    KeywordName = item.KeywordName,
                    NewsTypeShow = NewsTypeShow,
                    NewsKeyTagsOrder = Convert.ToInt32(item.OrderItem),
                    NewsKeyGroupsOrder = Convert.ToInt32(item.GroupOrder)
        ...
        protected void rptGroupNewKeyword_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            if (data != null)
            {
                if (GroupNewsList.Count != 0)
                {
                    ...
                    rptGroupNewsList.DataSource = GroupNewsList.Where(n => keyIds.Contains(n.KeywordId) && n.NewsKeyGroupsOrder == data.GroupOrder).OrderBy(t => t.OrderItem).ThenByDescending(d => d.NewsDateIndex).ThenByDescending(d => d.NewsTime).ToList();
                    rptGroupNewsList.DataBind();
                }
            }
        }
