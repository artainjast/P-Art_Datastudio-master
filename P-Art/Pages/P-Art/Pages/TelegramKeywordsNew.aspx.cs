...
{
    public partial class TelegramKeywordsNew : System.Web.UI.Page
    {
        ...
        List<Tbl_TLPKeywords> TelegramKey = new List<Tbl_TLPKeywords>();
        ...
        private void prepareDropDownList()
        {
            NewsGroupList = (from Group in _db.Tbl_NewsGroup
                             where UserPanelList.Contains(Group.ParminId) && Group.GroupType == 3 && Group.Active == true
        ...
        protected void OnItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                TelegramKey = (from key in _db.Tbl_TLPKeywords
        ...
        [WebMethod]
        public static Tbl_TLPKeywords GetTelegramKeywordWithId(string KeyID)
        {
            int keyId = int.Parse(KeyID);
            var keyWord = _dbStatic.Tbl_TLPKeywords.Where(key => key.ID == keyId).FirstOrDefault();
            return keyWord;
        }
        ...
        protected void deleteGroupButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var Keyword = _db.Tbl_TLPKeywords.Where(key => key.KeysGroupID == groupId && key.Active == true).FirstOrDefault();
                if (Keyword == null)
                {
        ...
        protected void saveKeyword_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var keyword = _db.Tbl_TLPKeywords.Where(k => k.ID == keyId).FirstOrDefault();
                if (keyword != null)
                {
                ...
                else
                {
                    if (!string.IsNullOrWhiteSpace(txtKeywordTitle.Text))
                    {
                        _db.Tbl_TLPKeywords.Add(new Tbl_TLPKeywords
                        {
