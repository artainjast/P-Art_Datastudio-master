...
{
    public partial class SocialBultanArchive : System.Web.UI.Page
    {
        ...
        protected void btn_Show_Click(object sender, EventArgs e)
        {
            ...
            rpt_result.DataSource = _cls.GetSocialBultanByDate(fromdate, todate, parminId).OrderByDescending(t => t.SocialMediaBultanID).ToList();
            rpt_result.DataBind();
        }
        ...
        public static string GetBultanFilePath(string path, object sender)
        {

            HiddenField hddsocialkeyid = sender as HiddenField;

            if (path.ToLower().Trim().StartsWith("http://") || path.ToLower().Trim().Contains("www."))
            {

                return string.Format("http://localhost:1629/HtmlSocialNews.aspx?ArchiveId="+ hddsocialkeyid.Value);
            }
            else
            {
