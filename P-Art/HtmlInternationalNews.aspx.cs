...
namespace P_Art
{
    public partial class HtmlInternationalNews : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                arzHtml = LoadArz();
                simaHtml = LoadSima();
                ltNewsHtml.Text = LoadFixedHeader() + jeldHtml + galleryNewspaper + newsPaperHtml + arzHtml + simaHtml + newsHtml;
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
            ...
            var MaxPageLength = 2700;
            var MaxPageNewLength = 3800;
                ...
                try
                {
                    ...
                    foreach (var news in lstNews.Where(t => t.GroupOrder == item.GroupOrder).ToList())
                    {
                        ...
                        @" <span class='circleSing'></span>                        
                            <a class='newsTitle' data-newsid='" + news.NewsID + @"'>" + news.NewsTitle + " " + relatedText + @"</a></div>
...
#pragma warning restore CS0168 // The variable 'ex' is declared but never used
        ...
        //public List<Tbl_News_Type> GenerateNewsTags(List<Tbl_News_Type> items, List<int?> panelIds)
        //{
        ...
        //            item.GroupName = (orderedKey.GroupName + "");
        //            try
        //            {
        //                item.OrderItem = Convert.ToInt32(orderedKey.OrderItem);
        //            }
        //            catch { item.OrderItem = 0; }
